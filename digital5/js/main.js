window.onload = function() {
       ...
       */
	   ...
	   deckPool1 = game.add.group();
        for(var i = 0; i < 16; i++)
		{
			...
			game.physics.enable( card1, Phaser.Physics.ARCADE );
	...
	function move(card1)
	{
		//console.log("Fuck ERROROR");
		if(card1.id == 1)
		{
			card1.position.x = 60 + (sword1.length * 85);
			if(card1.position.y < 318)
			{
				card1.body.velocity.y = 400;
			}
			if(card1.position.y >= 309)
			{
				combat = true;
				//console.log("THIS SHOULD WORK");
				card1.body.velocity.y = 0;
			}
		}
		if(card1.id == 2)
		{
			card1.position.x = 60 + (bow1.length * 85);
			if(card1.position.y < 218)
			{
				card1.body.velocity.y = 400;
			}
			if(card1.position.y >= 209)
			{
				combat = true;
				//console.log("THIS SHOULD WORK");
				card1.body.velocity.y = 0;
		...
		if(card1.id == 3)
		{
			...
			if(card1.position.y < 118)
			{
				card1.body.velocity.y = 400;
			}
			if(card1.position.y >= 109)
			{
				combat = true;
				//console.log("THIS SHOULD WORK");
				card1.body.velocity.y = 0;
